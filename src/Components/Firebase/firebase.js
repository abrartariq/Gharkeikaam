...
class Firebase {
  ...
  doSaveNewWorker = async information => {
    ...
    this.database
      ...
      .then(() => {
        console.log("Worker Successfully Saved");
      })
      .catch(error => {
  ...
  getworkdetails = async worker_type => {
    try {
      let workerdetails = {};
      const snapshot = await this.database.collection("Workers").get();
      snapshot.docs.map(doc => {
        const datas = doc.data();
        if (worker_type == datas["service"]) {
  ...
  doDisplayRequest = async email => {
    var snapshot = await this.database
      .collection("CurrentRequests")
      .where("customer_id", "==", email.hashCode())
      .get();
    let req_list = [];
